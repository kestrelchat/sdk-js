...
export type $FetchHandlerCallback<T> = (res: Response, err?: (reason: unknown) => PromiseLike<never>) => Promise<T>;

export async function FetchHandler<T>(src: ModularRequest | string, callback?: $FetchHandlerCallback<T>): Promise<T | undefined> {
  ...
  return await fetch(url, init)
    ...
    .then((data) => { // Determines cause of failure (HTTP error)
    ...
    .catch((reason) => { // Prevents unhandled exception (fetch error)
